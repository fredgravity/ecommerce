...
class UserPaymentController extends BaseController
{
    ...
    public function showPayments(){
        $revenues = Capsule::table('payments')->where('user_id', user()->id)->selectRaw('sum(amount) as `amount`')->get();
        $revenue = $revenues[0]->amount;
        return view('/user/products/userPayment', compact('revenue'));
    }
    ...
    public function paymentConverter(){
        if (Request::exist('post')){
            ...
            $payments = Capsule::table('payments')->where('user_id', user()->id)->select(
