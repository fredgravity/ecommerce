(function () {
    ...
    ARTISAOSTORE.nav.categoryItem = function () {
        var app = new Vue({
            el: '#root',
            data: {
                featured: [],
            ...
            methods: {
                ...
                getFeaturedProducts: function () {
                    ...
                    axios.all([
                        ...
                        axios.get('/featured'),
                        axios.post('/get-categoryitems', postData)

                    ]).then(axios.spread(function (featuredResponse, categoryitemResponse) {
                        //GET THE ECHOED JSON RESPONSE FROM INDEX CONTROLLER
                        app.featured = featuredResponse.data.featured;
                        app.categoryItems = categoryitemResponse.data.categoryItems;
                        app.count = categoryitemResponse.data.count;
                        app.loading = false;
                ...
                loadMoreProducts: function () {
                    this.loading = true;
                    var token = $('.display-product-search').data('token');
                    ...
                    axios.post('/load-more-categoryItems', data).then(function (response) {
                        app.categoryItems = response.data.categoryItems;
                        app.count = response.data.count;
                        app.loading = false;
                        // console.log(app.categoryItems);
                    })
                }
